...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		GameOverSubstate.daBf = SONG.player1;
		...
		if(curStage == 'calamityspace' || curStage == 'finalbattle' || curStage == 'bossmodestage') {
			introSoundsSuffix = '-shard';
		}
		...
		add(dadGroup);
		if(curStage == 'calamityspace' || curStage == 'bossmodestage'){
			dadGroup.scrollFactor.set(0.1, 0.1);
			tweenCamIn();
			var spaceFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('spacefront'));
			spaceFront.setGraphicSize(Std.int(spaceFront.width * 1.0));
			spaceFront.updateHitbox();
		...
		dadGroup.scrollFactor.set(-0.1, 0);
		boyfriendGroup.setGraphicSize(Std.int(boyfriendGroup.width * 2.4));
		boyfriendGroup.scrollFactor.set(0.3, 0);
		...
		if(curStage == 'calamityspace' || curStage == 'bossmodestage'){
			boyfriendGroup.y += 120;
			boyfriendGroup.x += 750;
		}
		if(curStage == 'finalbattle'){
		...
		if(curStage == 'caveday' || curStage == 'bossmodestage'){
		...
		iconP1.y = healthBar.y - (iconP1.height / 2) + 80;
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
				case 'no-newton':
					startVideo('NoNewtonCS720');
					//spaceIntro(doof);
				case 'calamity':
					startVideo('CalamityCS720');
					//spaceIntro(doof);
				case 'deities':
					startVideo('DeitiesCS720');
				case 'resurrection':
					startVideo('ResCS720');
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function undisturbedIntro(){
		inCutscene = true;
		startDialogue(dialogueJson);
	}
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				introAssets.set('shard', ['ready-shard', 'set-shard', 'go-shard']);
				...
				if(curStage == 'calamityspace' || curStage == 'finalbattle') {
					introAlts = introAssets.get('shard');
				}
					...
					switch (swagCounter)
					{
						case 0:
							if(curStage != 'bossmodestage'){
								...
								VSIconP2 = new HealthIcon(dad.healthIcon, true);
								//VSIconP2.screenCenter();
								VSIconP1 = new HealthIcon(boyfriend.healthIcon, true);
	...
	override public function update(elapsed:Float)
	{
		...
		if(curStage == 'bossmodestage'){
		iconP1.scale.set((mult * .95), (mult * .95));
		iconP1.updateHitbox();
		}
		else{
		...
		if (curStage == 'bossmodestage'){
		iconP1.x = healthBar.x - 30;
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - iconOffset - 60;
		}
		else{
		...
		if (curStage != 'bossmodestage'){
			...
			if (health >= 2){
				...
				var achieve:String = checkForAchievement(['beat_bossmode']);
				//if (achieve != null) {
				//	startAchievement(achieve);
				//}
				#end
				openSubState(new BossModeOverSubstate());
			}
			if (bossModeHealth < 2){
				iconP1.animation.curAnim.curFrame = 1;
			}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if(curStage != 'bossmodestage'){
			...
			{
				if(curStage == 'finalbattle' && immortalityReady == true){
			...
			if (((skipHealthCheck && instakillOnMiss) || bossModeHealth <= 0) && !practiceMode && !isDead)
			{
					GameOverSubstate.daBf = SONG.player1;
					GameOverSubstate.resetVariables();
					var ret:Dynamic = callOnLuas('onGameOver', []);
		...
		else
		{
			...
			switch (curStage)
			{
				...
				case 'bossmodestage':
					...
					camFollow.y = boyfriend.getMidpoint().y - 400;
				case 'caveday':
					camFollow.x = boyfriend.getMidpoint().x - 1490;
					camFollow.y = boyfriend.getMidpoint().y - 650;
			}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_complete', 'week1_nomiss', 'beat_bossmode', 'undis_done', 'undis_nomiss']);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!PlayState.isPixelStage)
		{
			if(curStage != 'calamityspace' && curStage != 'finalbattle' && curStage != 'bossmodestage' && curStage != 'bonus'){
			rating.setGraphicSize(Std.int(rating.width * 0.7));
			rating.antialiasing = ClientPrefs.globalAntialiasing;
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 0.7));
			comboSpr.antialiasing = ClientPrefs.globalAntialiasing;
			}
			else if (curStage == 'calamityspace' || curStage == 'bossmodestage' || curStage == 'bonus'){
			rating.setGraphicSize(Std.int(rating.width * 0.3));
			rating.y += 100;
			rating.antialiasing = ClientPrefs.globalAntialiasing;
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 0.2));
			comboSpr.antialiasing = ClientPrefs.globalAntialiasing;
			}
			else{
			rating.setGraphicSize(Std.int(rating.width * 0.3));
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = coolText.x + (27 * daLoop) - 90;
			}
			numScore.y += 80;
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			if (curSong.toLowerCase() == 'undisturbed' && curBeat == 190)
			{
				...
				defaultCamZoom = .3;
				dadGroup.setPosition(-1200, -200);
				dadGroup.scrollFactor.set(-0.10,0);
				//camPos.x += 300;
				//dadGroup.y += 400;
				//dadGroup.x -= 600;
				boyfriendGroup.setPosition(970, -70);
				boyfriendGroup.scrollFactor.set(0.3, 0);
		...
		if (curSong.toLowerCase() == 'calamity-boss-mode'){
				if(curBeat == 32){
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'week1_complete':
						var weekName:String = WeekData.getWeekFileName();
						if(weekName == 'week1' && isStoryMode && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice){
							unlock = true;
						}
					case 'beat_bossmode':
						if(Paths.formatToSongPath(SONG.song) == 'calamity-boss-mode' && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty && !usedPractice){
							unlock = true;
						}
					case 'undis_done':
						if(Paths.formatToSongPath(SONG.song) == 'undisturbed' && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty && !usedPractice){
							unlock = true;
						}
					case 'undis_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'undisturbed' && songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty && !usedPractice){
							unlock = true;
						}
					/*case 'ur_bad':
