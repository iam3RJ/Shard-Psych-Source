...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl.frames = Paths.getSparrowAtlas('vsshardlogo');
		...
		var shardMenu:FlxSprite = new FlxSprite(-650, -400).loadGraphic(Paths.image('shardMenu'));
		shardMenu.antialiasing = true;
		shardMenu.setGraphicSize(Std.int(bg.width * 1.05));
		shardMenu.active = false;
		add(shardMenu);
		add(logoBl);
		...
		threerjlogo = new FlxSprite(0, 0).loadGraphic(Paths.image('threerjlogo'));
		add(threerjlogo);
		threerjlogo.visible = false;
		threerjlogo.setGraphicSize(Std.int(threerjlogo.width * 0.8));
		threerjlogo.updateHitbox();
		threerjlogo.screenCenter(X);
		threerjlogo.screenCenter(Y);
		threerjlogo.antialiasing = true;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			// credTextShit.text += '\nVS';
			case 14:
				addMoreText('Shard'); // credTextShit.text += '\nShard';
